...
using BorgioBase.Dominio1.Utilitarios;
    ...
    class PreparaDbcontexto : DropCreateDatabaseAlways<DbContexto>
    {
        ...
        private static void PopularBaseDados(DbContexto contexto)
        {
            try
            {
                ...
                //PopularUsuarios(contexto);
                //PopularOperadora(contexto);
                //PopularPessoas(contexto);
                //PopularPF(contexto);
                //PopularEmpresas(contexto);
                //contexto.Commit();
                //PopularColaboradores(contexto);
                //PopularPlanoSaude(contexto);
                //PopularTitular(contexto);
                //PopularDependente(contexto);
                //contexto.Commit();
                //PopularPropostas(contexto);
